...
func (f *fakeQueryServiceServer) SearchLogs(req *pb.SearchLogsRequest, stream grpc.ServerStreamingServer[pb.SearchLogsResponse]) error {
	if len(f.logsToReturn) > 0 {
		if err := stream.Send(&pb.SearchLogsResponse{Logs: f.logsToReturn}); err != nil {
			return err
		}
	}
	return nil
...
func TestQueryServer_SearchLogs(t *testing.T) {
	...
	pb.RegisterQueryServiceServer(grpcServer, fakeSink)
